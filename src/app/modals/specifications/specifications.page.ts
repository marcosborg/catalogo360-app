...
export class SpecificationsPage implements OnInit {
  ...
  getLanguage() {
    checkName('language').then((resp) => {
      ...
      this.translation = this.languageService.getSpecifications(this.language);
    });
  }
  ...
  async addToCart(feature_id, feature_name, variation_name, price) {
    const alert = await this.alertController.create({
      header: this.translation.quantidade,
      inputs: [
        {
          name: 'qty',
          type: 'number',
        ...
        },
        {
          name: 'obs',
          type: 'textarea',
          placeholder: this.translation.observacoes
        }
      ],
      buttons: [
        {
